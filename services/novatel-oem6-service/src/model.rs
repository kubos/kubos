...
// The OEM-400 sends a set of telemtery messages every 250ms
// This function continuously reads them and updates the persistent data structs
pub fn read_thread(mai: OEM400, data: Arc<ReadData>, sender: Sender<String>) {
    let mut err_count = 0;
    loop {
    ...
    pub fn noop(&self) -> Result<GenericResponse, Error> {
        ...
        let (success, errors) = match new_ctr != old_ctr {
            ...
            false => {
                push_err!(
                    self.errors,
                    "Noop: Unable to communicate with OEM400".to_owned()
                );
                (false, "Unable to communicate with OEM400".to_owned())
            }
        };
