...
fn main()-> EpsResult<()> {
    ...
    let wd_timeout = config
        .get("wd_timeout")
        .ok_or_else(|| {
            error!("Failed to load 'wd_timeout' config value");
            format_err!("Failed to load 'wd_timeout' config value");
            println!("Failed to load wd_timeout");
        })
        .unwrap();
    let wd_timeout = wd_timeout.as_integer().unwrap() as u32;
    println!("wd_timeout = {}", wd_timeout);

    Service::new(
        config,
        Subsystem::new(bus,addr,wd_timeout)?,
        QueryRoot,
        MutationRoot,
    )
    .start();
