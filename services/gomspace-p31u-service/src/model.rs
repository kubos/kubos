...
impl Subsystem {
    ...
    pub fn eps_set_system_config(&self,    
                ppt_mode: i32,
                battheater_mode: i32,
                battheater_low:i32,
                battheater_high:i32,
        ...
        for i in 0..2 {
            sysconf.vboost[i]=vboost_settings[i] as u16;
        }
       
        let result = run!(self.eps.lock().unwrap().configure_system(sysconf), self.errors);
    ...
    pub fn eps_set_battery_config(&self,    
            batt_maxvoltage: i32,
        ...
        let result = run!(self.eps.lock().unwrap().configure_battery(battconf), self.errors);
    ...
    pub fn eps_set_output(&self, mask:i32) -> EpsResult<GenericResponse> {
        ...
        let result = run!(self.eps.lock().unwrap().set_output(channel_mask), self.errors);
    ...
    pub fn eps_set_single_output(&self, eps_channel:i32, eps_value:i32,eps_delay:i32) ->  EpsResult<(GenericResponse)> {
        ...
        println!("Service: Setting power state: Channel {} is set to {} with {}ms delay", channel, value,delay);
