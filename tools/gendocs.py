...
def gendocs_xml(dir, doxyfile, version, doc_dir, doc_tags):
    tags_str = make_tags_str(dir, doc_tags)
    print tags_str
    doxycmd = GENERATE_XML.format(doxyfile, version, doc_dir, tags_str)
...
def main():
    ...
    for dir in doc_dirs:
        ...
        if not os.path.isdir(doc_dir):
            os.makedirs(doc_dir)
        gendocs_xml(dir, "docs/Doxyfile", args.version, doc_dir, doc_tags)
