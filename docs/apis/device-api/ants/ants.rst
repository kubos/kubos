...
There are several options which may be specified within a project's ``config.json`` file under the ``antenna.isis`` structure. 
These option values should match what was specified in your antenna's options sheet. 
...
.. json:object :: antenna.isis

    Antenna System Configuration
    
    :property string primary: `(Default: "0x31")` I2C address of the primary antenna microcontroller
    :property string secondary: `(Default: "0x00")` I2C address of the secondary antenna microcontroller.
        A value of ``0x00`` indicates that no secondary microcontroller is available to the system
    :property integer count: `(Default: 2)` The number of deployable antennas available to the system
    :property watchdog: Watchdog configuration
    :proptype watchdog: :json:object:`watchdog <antenna.isis.watchdog>`

    
.. json:object:: antenna.isis.watchdog

    Watchdog properties
    
    :property integer timeout: `(Default: 60)` Antenna system's watchdog timeout period (in seconds). 
        If the system does not receive any I2C commands in this time period, it will restart itself. 
