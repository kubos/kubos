    ...
    let data = match message.payload.get(0) {
        ...
        _ => {
            return Err(ProtocolError::MessageParseError {
                err: "No stderr data found".to_owned(),
            })
        }
    };
...
/// Stderr -> CBOR
pub fn to_cbor(channel_id: u32, data: Option<&str>) -> Result<Vec<u8>, String> {
    info!("-> {{ {}, stderr, '{:?}' }}", channel_id, data);

    ser::to_vec_packed(&(channel_id, "stderr", data))
        .map_err(|_err| "Error creating stderr message".to_owned())
}
